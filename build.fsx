...
let preReleaseVersion = version + "-beta72"
...
let slnFile = "./src/Akka.Persistence.Oracle.sln"
...
Target "RunTests" <| fun _ ->  
    ...
    xUnit2
        (fun p -> { p with XmlOutputPath = Some (testOutput @@ "xunit.xml"); HtmlOutputPath = Some (testOutput @@ "xunit.html"); ToolPath = xunitToolPath; })
        xunitTestAssemblies
...
let updateNugetPackages _ =
  ...
  let getPackages project =
    match project with
    | "Akka.Persistence.Oracle" -> ["Akka.Persistence";"Akka.Persistence.Sql.Common"]
    | "Akka.Persistence.Oracle.Tests" -> ["Akka.Persistence.TestKit";"Akka.Persistence.Sql.Common";]
    | _ -> []
